globals [ actors opinions range-op all-pairs num-prim-actors num-sec-actors]

turtles-own [ opinion sources targets next-opinion coeff ]
...
to go
  ...
  if Actor-type = "Source" [
    ...
    ask actors [
      ...
      if (num-prim-actors < N) [ask targets [
        set sources (turtle-set sources myself) ; Set S_j
      ] ]
    ]
    if (num-prim-actors = N) [ask turtles [
      set sources other turtles ; Set S_j
    ] ]
  ]
  if Actor-type = "Target" [
    set actors ifelse-value (num-prim-actors = N)
      [turtles] [n-of num-prim-actors turtles] ; Set S
    ask actors [
      set sources ifelse-value (num-sec-actors = N - 1)
        [other turtles][n-of num-sec-actors other turtles] ; Set S_j
    ]
  ]
  if Actor-type = "Group" [
  ...
  set range-op ( max opinions - min opinions )
  tick
  if range-op < 0.01 [ stop ]
end

to go-sync
  let target-set no-turtles ; set of agents with |S_j|>0
  if-else Actor-type = "Source" [
    set target-set ifelse-value (num-sec-actors = N - 1)
      [turtles] [turtle-set [targets] of actors]
  ] [
    set target-set ifelse-value (num-prim-actors = ifelse-value (Actor-type = "Target") [N] [length all-pairs])
      [turtles] [turtle-set actors]
  ]
  ask target-set [
...
to go-async
  ...
  if Actor-type = "Source" [
    ...
    foreach sourcelist [ [this-source] ->
      ask this-source [ ask targets [ ; Set T_i
  ...
  if Actor-type = "Group" [ ; Note, this only works for s=2
...
The Generalized Repeated Averaging Model (GRAM) is a generalization of the repeated averaging opinion dynamics models originally proposed by Harary (1959) and expanded upon by Abelson (1964) and DeGroot (1974) to allow modification of the agent schedule.

## HOW IT WORKS

Agents are first randomly paired according to the specified schedule. Then, according to the schedule, all pairs exert influence. Specific equations are included in the associated article.
...
Click Go to run model until clusters have converged to within 0.01.

## THINGS TO OBSERVE

The Results plot shows the mean opinion over time and the range in opinions over time.
The Mean Opinion monitor displays the mean opinion of all agents in the model.

## REPLICATING RESULTS

Results from the associated paper may be replicated by using the BehaviorSpace tool (in the top menu, under Tools). AsyncS2 runs the Asynchronous Target/Source/Group (2,1000) results. SyncS2 runs the Synchronous Target/Source/Group (2,1000) results. AsyncSinfty runs the Asynchronous Target/Source (infinity,1000) results. SyncSinfty runs the Synchronous Target (infinity,1000) results, which is equivalent to Synchronous Source (infinity,1000).
...
- Abelson, R.P. (1964). Mathematical models of the distribution of attitudes under controversy. In N. Frederiksen & L. L. Thurstone (Eds.), Contributions to Mathematical Psychology, (pp. 142–160). New York, NY: Holt, Rinehart, and Winston
- DeGroot, M. H. (1974). Reaching a consensus. Journal of the American Statistical Association, 69(345), 118–121. doi:10.2307/2285509. http://dx.doi.org/10.2307/2285509
...
@#$#@#$#@
<experiments>
  <experiment name="SyncS2" repetitions="1000" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="AsyncS2" repetitions="1000" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean [opinion] of turtles</metric>
    <steppedValueSet variable="mu" first="0.01" step="0.01" last="0.99"/>
  ...
  <experiment name="SyncSinfty" repetitions="1000" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean [opinion] of turtles</metric>
    <steppedValueSet variable="mu" first="0.01" step="0.01" last="0.99"/>
  ...
  <experiment name="AsyncSinfty" repetitions="1000" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>mean [opinion] of turtles</metric>
    <steppedValueSet variable="mu" first="0.01" step="0.01" last="0.99"/>
